...
| onderdelen | type | RegEx | voorbeeld | Toelichting |
| --- | --- | --- | --- | --- |
| **middel** | string{4} | `RBNL\|BIO\|2FA\|geen` | RBNL | RBNL = Nederlands rijbewijs <br/>BIO = Biometrie <br/>2FA = 2-factor authenticatie <br/>geen = chauffeur is niet geauthenticeerd (NB: alleen te gebruiken bij aanmelden dienst door vervoerder bij aanlevering achteraf). |
| **kenmerk** | string(32) |     | `1234565677` `vingerafdruk` `gezichtsherkenning` `wachtwoord-SMS` `pincode-koppelcode` `geen` | Bij RBNL → rijbewijsnummer. <br/>Bij BIO → vingerafdruk, gezichtsherkenning. <br/>Bij 2FA → wachtwoord-SMS, wachtwoord-koppelcode, pincode-SMS, pincode-koppelcode. <br/>Bij geen → geen. |
...
| **validatiemethode** | string (1) | `K\|N` | `K` | "K": kentekenkaart gelezen <br/>"N": niet gevalideerd |
...
| onderdelen | type | RegEx | voorbeeld | Toelichting |
| --- | --- | --- | --- | --- |
| **breedtegraad** | string | `^\[-+\]?(90(.0{4,6})? \| (\[1-\]?\\d(.\\d{4,6})?))$` | `5.100056` | geografische breedtegraad met 4 tot 6 cijfers achter de punt |
| **lengtegraad** | string | `^\[-+\]?(180(.0{4,6})? \| ((1\[0-7\] \| \[1-9\])?\\d(.\\d{4,6})?))$` | `52.086491` | geografische lengtegraad met 4 tot 6 cijfers achter de punt |
